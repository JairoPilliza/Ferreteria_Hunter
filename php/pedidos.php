...
class Pedidos{
    ...
    public function insert($item = array())
    {
        if (!is_array($item) or count($item) === 0) {
            ...
            if (!isset($item['id'],$item['producto'],$item['price'],$item['cantidad'],$item['stock'])) {
                return false;
            }else{
        ...
        if(!is_array($item) or count($item) ===0){
            ...
            if(!isset($item['rowid'], $this->cart_contents[md5($item['rowid'])])){
                ...
                if(isset($item['cantidad'])){
                    ...
                    $b=$item['rowid'];
                    $query=$db->query("select * from inventario where invId=$b");
                    $row=$query->fetch_assoc();
                    $a=$row['invStock'];
                    if($item['cantidad']>$a||$item['cantidad']==0){
