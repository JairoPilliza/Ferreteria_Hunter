...
<?php
    $query="select * from orden o,clientes c where o.cliId=c.cliId order by o.ordId desc";
    $enviar=mysqli_query($db,$query);
    $ver=mysqli_fetch_array($enviar);
    echo "<center><h1 style=color:white;>Facturación</h1></center>";
    ...
    do{
    $id=$ver['ordId'];
    $nombre=$ver['cliNombre'];
    $fecha=$ver['ordFecha'];
    $total=$ver['ordTotalprecio'];
    echo '
        <tbody>
        <tr>
        <td>'.$id.'</td>
        ...
        <td>'.$fecha.'</td>
        <td>'.$total.'</td>
        <td><a href="pdf/index.php?id='.$id.'" class="btn btn-dark" target="_BLANK"><i class="fas fa-file-pdf"> Imprimir</i></a>
        </td>
        </tr>
